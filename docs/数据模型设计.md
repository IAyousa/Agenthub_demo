...
| 组件 | 状态 | 说明 |
|------|------|------|
| 4 个 JPA Entity 类 | ✅ 已完成 | 映射 `users`, `conversations`, `messages`, `agents`, `conv_agents` 五张表 |
| 4 个 JPA Repository 接口 | ✅ 已完成 | 继承 `JpaRepository`，含自定义查询方法 |
| 种子数据 `data.sql` | ✅ 已完成 | 启动时预填充 2 个 Agent |
| `WebSocketController` 中的数据库调用 | ❌ 未实现 | `handleUserMessage()` 方法体中仅有注释占位 |
| Service 层 | ❌ 未创建 | 业务逻辑层尚未开发 |

> ⚠️ **当前局限**：Repository 层已具备完整的读写能力，但尚无业务代码实际调用它们。后续开发 Service 层和 Controller 层时，直接 `@Autowired` 注入 Repository 即可使用。
